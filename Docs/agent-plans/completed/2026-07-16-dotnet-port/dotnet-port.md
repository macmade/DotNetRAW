# Port SwiftRAW to C# / .NET 10

Port SwiftRAW, a RAW file parser written in Swift, to idiomatic C# targeting .NET 10.

The original Swift repository is checked out at: `../SwiftRAW`
...
1. Port the complete SwiftRAW implementation to C#.
...
Keep names, responsibilities, and relationships between types reasonably consistent with the Swift project so that users familiar with SwiftRAW can recognize the C# API.
...
Match normal .NET naming conventions but match formatting conventions from SwiftRAW.
...
Use exceptions for error handling, as in SwiftRAW.

All parsing and formatting that is part of the RAW format must be deterministic and independent of the machine’s current culture. Use explicit culture and comparison rules where relevant.

Exact textual output does not need to match where the difference is merely a normal language-runtime convention.  
For example, Swift may represent a floating-point value as "42.0" in a string, whereas the default C# representation is "42". Such differences are acceptable unless the text is part of a specified serialization format, a RAW requirement, or behavior explicitly tested by the original project.  
...
SwiftRAW uses LibRAW with the C++ API.  
A Windows version of LibRAW is available in the `LibRAW` directory.

Since LibRAW also exposes a C API, you will be able to use it directly from C#.  
Create an interop using the provided DLL (DLLImport + marshaling).
